...
bool Adom_Joystick::Movement() {
  ...
  if ( abs(this->Last_X) < abs(this->X()) - abs(this->Deadzone_X) )
    Serial.println("X moved");
  if ( abs(this->Last_Y) < abs(this->Y()) - abs(this->Deadzone_Y) )
    Serial.println("Y moved");

  Serial.print(digitalRead(this->Pin_X));
  Serial.print(" , ");
  Serial.println(digitalRead(this->Pin_Y));
